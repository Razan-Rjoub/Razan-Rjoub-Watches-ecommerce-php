...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['image2']) && $_FILES['image2']['error'] === UPLOAD_ERR_OK) {
        $image = file_get_contents($_FILES['image2']['tmp_name']);
        // $base64Image = base64_encode($image); html size is lower 
    } else {
        echo "Image upload error.";
        exit();
    ...
    if (isset($_FILES['image3']) && $_FILES['image3']['error'] === UPLOAD_ERR_OK) {
        $image = file_get_contents($_FILES['image3']['tmp_name']);
        // $base64Image = base64_encode($image); html size is lower 
    } else {
        echo "Image upload error.";
        exit();
    ...
    $insertQuery = "INSERT INTO product (Productname, description, price, stockqty, categoryid, color, image,image2,image3)
                    VALUES (:Productname, :description, :price, :stockqty, :categoryid, :color, :image ,:image2,:image3)";
